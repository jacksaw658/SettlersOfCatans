...
import guiClasses.currentPlayerStats;
import players.City;
...
public class CatanWindow {
	 ...
	 */
	public CatanWindow(GameBoard gameBoard, int numPlayers, ArrayList<Player>playersgiven) {
		initialize(gameBoard,numPlayers,playersgiven);
		board = gameBoard;
	...
	private void initialize(GameBoard gameBoard, int numPlayers, ArrayList<Player> players) {
		frame = new JFrame();
		...
		// each dimension
		panel.setBounds(450, 120, width + BUFFER * 2, height + BUFFER * 2);
	 ...
	 *            visual representation for
	 */
	public void addPlayerFrames(int numPlayers, ArrayList<Player> players)
	{
		//adding player panels
		if(numPlayers ==2)
		{
		currentPlayerStats playerOne = new currentPlayerStats(players.get(0));
		frame.getContentPane().add(playerOne);
		playerOne.updatePlayer();
		playerOne.setVisible(true);
		playerOne.setBounds(20, 50, 300, 300);
		//playerOne.setBounds(1000, 0, width + BUFFER * 2, height + BUFFER * 2);
		currentPlayerStats playerTwo = new currentPlayerStats(players.get(1));
		frame.getContentPane().add(playerTwo);
		playerTwo.updatePlayer();
		playerTwo.setVisible(true);
		playerTwo.setBounds(1000, 500, 300, 300);
		}
		if(numPlayers==3)
		{
			currentPlayerStats playerThree = new currentPlayerStats(players.get(2));
			frame.getContentPane().add(playerThree);
			playerThree.setVisible(true);
			playerThree.updatePlayer();
			playerThree.setLocation(120,150);
			playerThree.setBounds(800, 0, width + BUFFER * 2, height + BUFFER * 2);
		}
		if(numPlayers==4)
		{
			currentPlayerStats playerFour = new currentPlayerStats(players.get(3));
			playerFour.updatePlayer();
			frame.getContentPane().add(playerFour);
			playerFour.setVisible(true);
			playerFour.setLocation(120,500);
			playerFour.setBounds(0, 600, width + BUFFER * 2, height + BUFFER * 2);
		}
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			...
			public void run() {
				try {
					CatanWindow window = new CatanWindow(board,numberOfPlayers,players);
